...
class RPR_RecipePostType extends AdminPageFramework_PostType {
	...
	public function setUp() {
		...
		$this->setArguments(
			array(
				'labels'        => array(
					...
					'plugin_listing_table_title_cell_link' => __( 'RPR recipe post type', 'recipepress-reloaded' ),
					// (framework specific key). [3.0.6+]
				),
				...
				'screen_icon'   => dirname( __FILE__ ) . '/img/logo_32x32.png',
				// a file path can be passed instead of a url, plugins_url( 'asset/image/wp-logo_32x32.png', APFDEMO_FILE )
				'menu_position' => 5,
				'has_archive'   => true,
				'taxonomies'    => $taxonomies,
				'rewrite'       => array(
					'slug' => sanitize_title( AdminPageFramework::getOption( 'rpr_options', array(
						'general',
						'slug',
					), 'recipe' ) ),
				),
		 ...
		 */
		$name = AdminPageFramework::getOption( 'rpr_options', array(
			'tax_builtin',
			'ingredients',
			'singular',
