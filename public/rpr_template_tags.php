	...
	function get_the_rpr_taxonomy_headline( $taxonomy, $icons = false ) {
		...
		if ( $icons ) {
			...
			if ( $taxonomy === 'category' || $taxonomy === 'post_tag' ) {
				$icon_class = esc_html( AdminPageFramework::getOption( 'rpr_options', array(
					'tax_builtin',
					$taxonomy,
					'icon_class',
				), 'fa-list-ul' ) );
...
if ( ! function_exists( 'get_the_rpr_taxonomy_terms' ) ) {
	...
	function get_the_rpr_taxonomy_terms( $taxonomy, $icons = false, $label = false, $sep = '&nbsp;/&nsbp;' ) {
		...
		if ( $taxonomy === 'category' ) {
			...
			$property_id = esc_html( AdminPageFramework::getOption( 'rpr_options', array(
				'tax_custom',
				$optkey,
				'property_id',
			), '' ) );
...
if ( ! function_exists( 'get_the_rpr_recipe_ingredients' ) ) {
	...
	function rpr_render_ingredient_line( $ingredient ) {
		...
		$data_format = AdminPageFramework::getOption( 'rpr_options', array(	'metadata',	'structured_data_format' ), 'microdata' );
		$auto_plural = AdminPageFramework::getOption( 'rpr_options', array(	'tax_builtin', 'ingredients', 'auto_plural'	), 0 );
		...
		if ( $data_format === 'microdata' ) {
			$out .= '<span itemprop="recipeIngredient" >';
		} elseif ( $data_format === 'rdfa' ) {
		...
		if ( isset( $ingredient['amount'] ) && $ingredient['amount'] > 1 && $ingredient['unit'] === '' && $auto_plural ) {
